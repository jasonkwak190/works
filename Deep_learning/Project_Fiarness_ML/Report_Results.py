...
def report_results(data):
    ...
    if demographic_parity_data is not None:
        ...
        print("")
        for group in demographic_parity_thresholds.keys():
            print("F-Score for " + group + ": " + str(calculate_Fscore(demographic_parity_data[group])))

        print("")
        rocData=[]
        for group in demographic_parity_thresholds.keys():
            rocData.append(get_ROC_data(data[group], group))
    ...
    if equal_opportunity_data is not None:
        ...
        print("")
        for group in equal_opportunity_data.keys():
            print("F-Score for " + group + ": " + str(calculate_Fscore(equal_opportunity_data[group])))
    ...
    if predictive_parity_data is not None:
        ...
        print("")
        for group in predictive_parity_data.keys():
            print("F-Score for " + group + ": " + str(calculate_Fscore(predictive_parity_data[group])))
    ...
    if single_threshold_data is not None:
        ...
        print("Total accuracy: " + str(total_accuracy))
        print("-----------------------------------------------------------------")
        plot_disparity_graph(demographic_parity_data,equal_opportunity_data,max_profit_data,predictive_parity_data,single_threshold_data)
        end = datetime.now()
